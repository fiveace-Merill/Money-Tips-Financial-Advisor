...
export function MessageBubble({ user, message }: Props) {
  ...
  return (
    <div className="flex items-start gap-4">
      <div className="h-9 w-9 mt-2 grow-0 bg-inherit border-[1px] border-background-container aspect-square rounded-full flex justify-center items-center">
        <Avatar >
      ...
      <div className="basis-11/12 p-2 bg-surface text-on-surface shadow-sm shadow-background/40 rounded-lg">
        <p className="p-3 text-md text-pretty lg:text-lg lg:text-justify">{message}</p>
      </div>
    </div>
  )
}
